<?php
    ...
    $username = $_POST['php_emp_user'];
    $password = $_POST['php_emp_pass'];
    ...
    $emp_sql_stmt = $connection->prepare("INSERT INTO employee (employee_ssn, first_name, last_name, address, phone, email, username, dept_id)
            VALUES (?, ?, ?, ?, ?, ?, ?, ?)");
    $emp_sql_stmt->bind_param("isssissi", $employee_ssn, $first_name, $last_name, $address, $phone, $email, $username, $dept_id);
    ...
    $emp_cred_sql_stmt = $connection->prepare("INSERT INTO employee_credentials (employee_ssn, username, password)
            VALUES (?, ?, ?)");
    $emp_cred_sql_stmt->bind_param("iss", $employee_ssn, $username, $password);
